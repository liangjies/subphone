...
import com.liangjies.subphone.entity.Item;
...
public class RedisController {
    ...
    /**
     * @Cacheable 应用到读取数据的方法上，先从缓存中读取，如果没获有再从DB取数据，然后把数据添加到缓存中
     *      * unless 表示条件表达式成立的话不放入缓存
     * @return
     */
    @GetMapping("/item/{content}")
    @Cacheable(value = "text")
    public Text getItemById(@PathVariable String content) {
        Text text = new Text();
